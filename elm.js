...
Elm.Timer.make = function (_elm) {
   "use strict";
   _elm.Timer = _elm.Timer || {};
   if (_elm.Timer.values)
   ...
   var green = A3($Color.rgb,
   133,
   153,
   0);
   var cyan = A3($Color.rgb,
   42,
   161,
   152);
   ...
   139,
   210);
   var violet = A3($Color.rgb,
   108,
   113,
   196);
   var magenta = A3($Color.rgb,
   211,
   54,
   130);
   var red = A3($Color.rgb,
   220,
   50,
   47);
   ...
   75,
   22);
   var yellow = A3($Color.rgb,
   181,
   137,
   0);
   var base3 = A3($Color.rgb,
   253,
   246,
   227);
   var base2 = A3($Color.rgb,
   238,
   232,
   213);
   var base1 = A3($Color.rgb,
   147,
   161,
   161);
   var base0 = A3($Color.rgb,
   131,
   148,
   150);
   var base00 = A3($Color.rgb,
   101,
   123,
   131);
   var base01 = A3($Color.rgb,
   88,
   110,
   117);
   var base02 = A3($Color.rgb,
   7,
   54,
   66);
   var base03 = A3($Color.rgb,
   0,
   43,
   54);
   var markerRadius = 20;
   var faceRadius = 120;
   var totalDiameter = 2 * (faceRadius + markerRadius);
   var setTimeUpdates = $Signal.mailbox($Graphics$Input$Field.noContent);
   var update = F2(function (action,
   model) {
      return function () {
         switch (action.ctor)
         {case "SetTime":
            return _U.replace([["setTime"
                               ,action._0]],
              model);
            case "TimeDelta":
            return _U.replace([["elapsed"
                               ,model.elapsed + action._0]],
              model);
            case "TimerState":
   ...
   var TimerState = function (a) {
      return {ctor: "TimerState"
             ,_0: a};
   };
   ...
   var updates = function () {
      ...
      var setTime = A2($Signal.filter,
      allDigits,
      $Graphics$Input$Field.noContent)(setTimeUpdates.signal);
   ...
   var model = function () {
      var initialState = {_: {}
                         ...
                         ,setTime: {_: {}
                                   ,selection: {_: {}
                                               ,direction: $Graphics$Input$Field.Forward
                                               ,end: 1
                                               ,start: 1}
                                   ,string: "4"}};
   ...
   var isFinished = function (state) {
      return _U.cmp(state.elapsed,
      total(state)) > 0;
   };
   var ringBell = Elm.Native.Port.make(_elm).outboundSignal("ringBell",
   function (v) {
      return v;
   },
   $Signal.dropRepeats($Signal.map(isFinished)(model)));
   var marker = function (state) {
      return function () {
         var shape = state.running ? A2($Graphics$Collage.ngon,
         3,
         markerRadius) : A2($Graphics$Collage.ngon,
         6,
         markerRadius / 2);
         var color = isFinished(state) ? base2 : base02;
         var fractionElapsed = state.elapsed / total(state);
         var locationAngle = $Basics.degrees(90) - $Basics.turns(fractionElapsed);
         var location = $Basics.fromPolar({ctor: "_Tuple2"
                                          ,_0: faceRadius
                                          ,_1: locationAngle});
         var markerAngle = $Basics.degrees(270) - $Basics.turns(fractionElapsed);
         return $Graphics$Collage.move(location)($Graphics$Collage.rotate(markerAngle)($Graphics$Collage.filled(color)(shape)));
      }();
   };
   var face = function (state) {
      return A3($Graphics$Collage.collage,
      totalDiameter,
      totalDiameter,
      _L.fromArray([A2($Graphics$Collage.filled,
                   blue,
                   $Graphics$Collage.circle(faceRadius))
                   ,marker(state)]));
   };
   var timer = F2(function (_v9,
   state) {
      return function () {
         switch (_v9.ctor)
         {case "_Tuple2":
            return function () {
                 var backgroundColor = isFinished(state) ? base02 : base2;
      ...
      return function () {
         ...
         {case "_Tuple2":
            return function () {
                 ...
                 var backgroundColor = finished ? base03 : base3;
                 var textStyle = A6($Text.Style,
                 _L.fromArray(["Source Code Pro"
                              ,"Courier New"
                              ,"monospace"]),
                 $Maybe.Just(16),
                 finished ? base00 : base0,
                 false,
                 false,
                 $Maybe.Nothing);
                 var fieldStyle = A4($Graphics$Input$Field.Style,
                 $Graphics$Input$Field.uniformly(0),
                 A3($Graphics$Input$Field.Outline,
                 blue,
                 $Graphics$Input$Field.uniformly(4),
                 0),
                 $Graphics$Input$Field.noHighlight,
                 textStyle);
                 return $Graphics$Element.color(backgroundColor)(A3($Graphics$Element.container,
                 _v13._0,
                 _v13._1,
                 $Graphics$Element.middle)($Graphics$Element.color(backgroundColor)(A3($Graphics$Input$Field.field,
                 fieldStyle,
                 $Signal.message(setTimeUpdates.address),
